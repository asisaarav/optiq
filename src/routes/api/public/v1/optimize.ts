...
function optimizeSql(code: string) {
  ...
  if (/\bSELCT\b/i.test(code))
    diagnostics.push({ severity: "error", message: "Typo: SELCT → SELECT" });
  if (/\bFORM\b/i.test(code)) diagnostics.push({ severity: "error", message: "Typo: FORM → FROM" });
  if (/SELECT\s+\*/i.test(output))
    changes.push({
      title: "Avoid SELECT *",
      detail: "Project only required columns to reduce scan and network I/O.",
    });
  if (/DATE\(\s*\w+\s*\)\s*=/i.test(output)) {
    ...
    changes.push({
      ...
      detail: "Removed a function from the filtered column so indexes/partition pruning can apply.",
      highlight: true,
    });
  }
  if (/\bUNION\b(?!\s+ALL)/i.test(output)) {
    ...
    changes.push({
      title: "UNION ALL",
      detail: "Avoids a distinct sort when duplicate removal is not required.",
    });
  }
  if (/LIKE\s+'%[^%']+%'/i.test(output))
    changes.push({
      title: "Search index",
      detail:
        "Leading wildcard LIKE cannot use a normal B-tree index; consider trigram/full-text indexes.",
    });
  if (/JOIN/i.test(output) && /WHERE/i.test(output))
    changes.push({
      title: "Join filtering",
      detail: "Push the most selective predicates before large joins and verify with EXPLAIN.",
    });
  if (!changes.length)
    changes.push({
      title: "Plan check",
      detail: "No safe rewrite found; inspect EXPLAIN for scans, spills, skew, and stale stats.",
    });
...
function optimizePython(code: string) {
  ...
  code.split("\n").forEach((line, idx) => {
    if (
      /^\s*(def|class|if|elif|else|for|while|try|except|finally|with)\b[^:]*$/.test(
        line.replace(/#.*$/, ""),
      ) &&
      line.trim()
    ) {
  ...
  if (/==\s*True\b/.test(output)) {
    output = output.replace(/\s*==\s*True\b/g, "");
    changes.push({ title: "Truthy check", detail: "Removed explicit == True." });
  }
  if (/for\s+\w+\s+in\s+range\(len\(\w+\)\):/.test(output)) {
    output = output.replace(
      /for\s+(\w+)\s+in\s+range\(len\((\w+)\)\):/g,
      "for $1, item in enumerate($2):",
    );
    changes.push({ title: "Use enumerate", detail: "Avoids repeated indexing and is clearer." });
  }
  if (/\w+\s*=\s*0\s*\n\s*for\s+\w+\s+in\s+\w+:/m.test(output))
    changes.push({
      title: "Built-in reductions",
      detail: "Use sum/min/max where possible to move loops into optimized runtime code.",
    });
  if (
    !/if\s+__name__\s*==\s*["']__main__["']\s*:/.test(output) &&
    /print\(|for\s|while\s|=/.test(output)
  ) {
  ...
  if (!changes.length)
    changes.push({
      ...
      detail: "No safe syntax-level rewrite found; profile hot paths and algorithmic complexity.",
    });
  return {
    output: `# Optimized by Optiq · PYTHON\n${output}`,
    diagnostics,
    changes,
    speedup: Math.min(78, 18 + changes.length * 9),
  };
}
